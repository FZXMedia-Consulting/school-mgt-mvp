...
// here everything seem to be ok, lets process our form		
...
function query($query){
	...
	return $que;
} // since this function now come from select_db.php we comment it out
*/
...
$numbers3 = '7856239014';
$letters4 = md5('SHJKABCDTUVWXLMNOPQRYZGEFI');
$numbers4 = '7239108564';
$date = new DateTime();
...
for($i = 1; $i <= $q; $i++) {
	...
	@$pn = strtoupper(str_shuffle(substr($letters, mt_rand(0, (strlen($letters) - $snpairs)), $snpairs)));
	...
	if($countCheck1 >= 1){
		...
		$check2= "SELECT COUNT(*) AS cnt FROM $tablename WHERE codec='$pn' OR sn='$sn'";
		...
		$check3= "SELECT COUNT(*) AS cnt FROM $tablename WHERE codec='$pn' OR sn='$sn'";
	...
	echo $errorh.' Something is not right, seems like ther is trouble with the database: '.mysql_error().$errort;
	}
}
?>
<div align="center">
...
<?php
//start the whole stuff: If there is a create Wallet Pin, there should be no table displayed
	if ($_GET['myaction'] != 'wallet' AND $_GET['myaction'] != 'registration') {
?>
      ...
      <table class="table table-striped table-bordered bootstrap-datatable datatable">
        ...
        <tbody>
          <?php
				...
				// start from 100 give me 2000 entries
...
//end the whole stuff: If there is a create Wallet Pin, there should be no table displayed
?>
<p>&nbsp;</p>